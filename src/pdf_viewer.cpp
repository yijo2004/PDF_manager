...
bool PdfViewer::Load(const std::string& filepath)
{
    ...
    m_document = FPDF_LoadMemDocument(m_pdfData.data(), static_cast<int>(m_pdfData.size()), nullptr);
    if (!m_document)
    {
...
void PdfViewer::ZoomIn(float factor)
{
    SetZoom(m_zoomLevel * factor);
}

void PdfViewer::ZoomOut(float factor)
{
    SetZoom(m_zoomLevel / factor);
}
...
void PdfViewer::RenderPageToTexture()
{
    ...
    if (renderWidth > MAX_TEXTURE_SIZE) renderWidth = MAX_TEXTURE_SIZE;
    if (renderHeight > MAX_TEXTURE_SIZE) renderHeight = MAX_TEXTURE_SIZE;
    ...
    // Render the page
    FPDF_RenderPageBitmap(bitmap, m_page, 0, 0, renderWidth, renderHeight, 0, FPDF_ANNOT);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, renderWidth, renderHeight, 
